...
def main():
    ...
    # Collection of hsv color to classify with several attributes (Ids from Palette 1 [0-19] to Palette 2 [20-39])
    data = [
    ...
    while(b != 'e'):
        ...
        distances = [norm(x["fhsv"],acquiredColor) for x in data]

        # Select the known color which is nearer wrt the acquired color
        mostLikelyColor = distances.index(min(distances))
